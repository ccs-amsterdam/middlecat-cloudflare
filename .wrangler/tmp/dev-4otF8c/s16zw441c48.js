// .wrangler/tmp/bundle-hMw3hQ/checked-fetch.js
...
// .wrangler/tmp/pages-abYbsV/bundledWorker-0.831594722951303.mjs
...
var Q = A((H) => {
  ...
  H.parse = ye;
  H.serialize = Re;
  var ge = Object.prototype.toString, k = /^[\u0009\u0020-\u007e\u0080-\u00ff]+$/;
  function ye(t, e) {
    if (typeof t != "string")
  ...
  function ve(t) {
    return ge.call(t) === "[object Date]" || t instanceof Date;
  }
  function _e(t, e) {
    try {
      return e(t);
  ...
  async set(e, r) {
    ...
    switch (await this.update(e, JSON.stringify(s)), s.value?.kind) {
      case "FETCH": {
        ...
        await this.setTags(a, { cacheKey: e }), q(a).forEach((n) => N.delete(n));
      }
    }
  }
  async get(e) {
...
function P(t, e, r) {
  ...
  let s = r ? "" : "i", a = [];
  return { match: (0, B.default)(`%${t}%${s}`, a).exec(e), captureGroupKeys: a };
}
...
u();
l();
function ue(t, { protocol: e, hostname: r, port: s, pathname: a }) {
...
function le(t, e) {
  ...
  if (!c && !e?.domains?.includes(o.hostname) && !e?.remotePatterns?.find((m) => ue(o, m)))
    return;
...
async function G(t, { buildOutput: e, assetsFetcher: r, imagesConfig: s }) {
  let a = le(t, s);
  if (!a)
...
function Z() {
  globalThis.fetch[J] || (pe(), globalThis.fetch[J] = true);
}
function pe() {
  let t = globalThis.fetch;
  globalThis.fetch = async (...e) => {
    let r = new Request(...e), s = await fe(r);
    return s || (s = await W(r), s) ? s : (me(r), t(r));
  };
}
async function fe(t) {
...
var E = class {
  ...
  applyRouteDest(e, r, s) {
    ...
    this.wildcardMatch && /\$wildcard/.test(n) && (n = n.replace(/\$wildcard/g, this.wildcardMatch.value)), this.path = x(n, r, s);
    ...
    return y(this.searchParams, j.searchParams), v(this.path) || (this.path = j.pathname), a;
  }
...
async function Y(t, e, r) {
  let s = new E(e.routes, r, t, e.wildcard), a = await ee(s);
  return be(t, a, r);
}
async function ee(t, e = "none", r = false) {
  return await t.run(e) === "error" || !r && t.status && t.status >= 400 ? ee(t, "error", true) : { path: t.path, status: t.status, headers: t.headers, searchParams: t.searchParams, body: t.body };
}
async function be(t, { path: e = "/404", status: r, headers: s, searchParams: a, body: n }, i) {
...
var Bt = { async fetch(t, e, r) {
  ...
  return s.run({ ...e, NODE_ENV: "production", SUSPENSE_CACHE_URL: w }, async () => {
    if (new URL(t.url).pathname.startsWith("/_next/image"))
      return G(t, { buildOutput: f, assetsFetcher: e.ASSETS, imagesConfig: p.images });
    let n = K(t);
...
// .wrangler/tmp/pages-abYbsV/s16zw441c48.js
  ...
  fetch(request, env, context) {
    ...
    for (const include of routes.include) {
      if (isRoutingRuleMatch(pathname, include)) {
        ...
        return Bt.fetch(request, env, context);
      }
    }
    return env.ASSETS.fetch(request);
  }
...
// .wrangler/tmp/bundle-hMw3hQ/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hMw3hQ/middleware-loader.entry.ts
